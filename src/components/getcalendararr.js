...
export default function getCalendarArr(now) {
  function getWeeksQuantity() {
    const start = new Date(
      moment().date(1).day(1).format("YYYY-MM-DD")
    ).getTime();
    const end = new Date(
      moment().endOf("month").day(1).format("YYYY-MM-DD")
    ).getTime();
  ...
  for (let index = 0; index < props.weeksQuantity; index += 1) {
    ...
    for (let i = 1; i < 8; i += 1) {
      let prop = null;
      const date = moment().date(1).day(i).add(index, "W").format("D");
